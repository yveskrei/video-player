"""FastAPI app: lifespan, CORS, media serving, the global /ws endpoint."""
import asyncio
import logging
...
from managers.websocket import manager as ws_manager, set_event_loop
...
async def lifespan(app: FastAPI):
    ...
    for d in [DASH_OUTPUT_DIR, PROGRESSIVE_OUTPUT_DIR]:
        if d.exists():
...
async def serve_dash_file(video_id: int, filename: str):
    file_path = DASH_OUTPUT_DIR / str(video_id) / filename
...
async def serve_progressive_init(video_id: int):
    ...
    file_path = PROGRESSIVE_OUTPUT_DIR / str(video_id) / "progressive.mp4"
    if not file_path.exists():
...
if __name__ == "__main__":
    ...
    port = int(os.getenv("APP_PORT", "8702"))
    # uvicorn's reloader cannot watch a subdirectory of the CWD: WatchFilesReload
    # drops any reload_dir under Path.cwd() and watches the CWD instead. So make
    # src/ the CWD — that scopes the watch to source only, keeping .venv/ and the
    # runtime data dirs out of it. Storage paths are absolute, so nothing else cares.
