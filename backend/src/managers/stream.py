"""FFmpeg orchestration: one subprocess per stream producing DASH + progressive fMP4."""
import logging
import os
...
# Custom modules
from utils.storage import storage, DASH_OUTPUT_DIR, PROGRESSIVE_OUTPUT_DIR
from utils.enums import StreamStatus, VideoUpdateReason
from managers.websocket import manager as ws_manager, broadcast_sync

# Variables
logger = logging.getLogger(__name__)
DASH_SEGMENT_DURATION = 2   # seconds
DASH_WINDOW_SIZE = 150      # 150 x 2s = 300s DVR window
# FFmpeg can spend seconds probing before its first segment, especially with
# -stream_loop -1 and -c:v copy.
INIT_TIMEOUT = 20

class ProgressiveHub:
    """One-producer-many-consumers fan-out for live fMP4 fragments. The stdout
    drainer is the sole producer; each HTTP client owns a bounded queue, so a slow
    consumer drops its own fragments without affecting FFmpeg or other clients.
    The latest fragment is cached so a joining client gets video immediately."""
...
class StreamManager:
    ...
    def _build_video_payload(video_id: int) -> dict:
        ...
        return {
            ...
            # Authoritative DVR capacity. dash.js's getDvrWindow() jitters and a
            # client-side constant drifts when the backend is retuned.
    ...
    def _monitor(video_id: int) -> None:
        ...
        # dash.js 5.1.1 races in StreamController._composePeriods: from a one-segment
        # MPD, addDVRMetric silently fails and STREAMS_COMPOSED never fires, so
        # playback never starts. 3 segments means ffmpeg has rewritten the MPD twice
        # with a real segment timeline.
    ...
    def start_stream(cls, video_id: int) -> None:
        ...
        with lock:
            ...
            # One libx264 encoder for DASH; the progressive branch is a remux
            # (-c:v copy). Giving it its own encoder cost ~2x CPU per stream.
            cmd = [
                "ffmpeg",
                "-v", "warning",
                # 50M/100M scanned far more than needed and delayed the first
                # segment by seconds, tripping INIT_TIMEOUT before the DVR filled.
                "-probesize", "5M",
                "-analyzeduration", "5M",
                ...
                # 60s of slack past the advertised window, covering the race between
                # dash.js fetching an oldest-edge segment and ffmpeg unlinking it on
                # roll-off. The earlier 10s margin produced the oldest-edge freeze.
